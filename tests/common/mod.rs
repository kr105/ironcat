...
use ironcat::script::signature::{signature_hash, SigHashType};
...
pub fn sign_tx(sk: &SigningKey, tx: &Transaction, input_idx: usize, script_code: &[u8]) -> Vec<u8> {
	let sighash = signature_hash(tx, input_idx, script_code, SigHashType::All);
	let sig: k256::ecdsa::Signature = sk.sign(sighash.as_bytes());
