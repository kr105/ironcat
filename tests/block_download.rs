...
fn setup() -> (
	...
	Arc<ChainState>,
	Arc<tokio::sync::RwLock<ironcat::mempool::Mempool>>,
	tempfile::TempDir,
) {
	...
	(nm, bs, rx, drx, rrx, cs, mempool, dir)
}

#[tokio::test]
async fn manager_starts_at_height_one() {
	let (nm, bs, rx, drx, rrx, cs, mp, _dir) = setup();
	let mgr = BlockDownloadManager::new(nm, bs, rx, drx, rrx, cs, mp);
...
async fn timeout_scan_removes_stale_entries() {
	let (nm, bs, rx, drx, rrx, cs, mp, _dir) = setup();
	let mut mgr = BlockDownloadManager::new(nm, bs, rx, drx, rrx, cs, mp);
...
#[tokio::test]
async fn handle_reorg_resets_download_state() {
	let (nm, bs, rx, drx, rrx, cs, mp, _dir) = setup();
	let mut mgr = BlockDownloadManager::new(nm, bs, rx, drx, rrx, cs, mp);
	...
	mgr.next_connect_height = 102;
	mgr.next_height = 200;

	let new_tip_height = 50;
	mgr.handle_reorg(new_tip_height);

	assert!(mgr.in_flight.is_empty());
	assert!(mgr.pending_blocks.is_empty());
	assert_eq!(mgr.next_connect_height, new_tip_height + 1);
}
